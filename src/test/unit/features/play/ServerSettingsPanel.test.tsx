...
} from '../../../../features/play/ServerSettingsPanel';
  ...
  return (
    <>
      <ServerSettingsPanel
        ...
        onMaxChatMessageCharactersChange={(maxChatMessageCharacters) =>
          setSettings((current) => ({
            ...current,
...
describe('ServerSettingsPanel', () => {
  ...
  it('saves only chat limits from 100 through 50,000', async () => {
    const user = userEvent.setup();
    render(<Harness />);
    const limit = screen.getByLabelText('Maximum chat message characters');

    await user.clear(limit);
    await user.type(limit, '25000{Enter}');
    expect(getSettings().maxChatMessageCharacters).toBe(25_000);

    await user.clear(limit);
    await user.type(limit, '99{Enter}');
    expect(limit).toHaveValue(25_000);
    expect(getSettings().maxChatMessageCharacters).toBe(25_000);

    await user.clear(limit);
    await user.type(limit, '50001');
    await user.tab();
    expect(limit).toHaveValue(25_000);
  });
