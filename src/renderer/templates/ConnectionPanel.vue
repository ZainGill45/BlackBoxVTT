...
const tabName = ref<TabName>("join");
...
<template>
  <section class="flex w-2/7 flex-col items-center gap-2 border border-neutral-600 bg-linear-to-b from-neutral-800 via-neutral-900 to-neutral-950 p-2 pb-2.5 shadow-lg shadow-black/50">
    ...
    <div class="flex w-full items-center">
      <DefaultButton buttonText="Join Game" :class="tabName === 'join' ? 'bg-neutral-950' : ''" class="w-full! h-10! text-sm! border-r-0! hover:border-r!" @click="tabName = 'join'" />
      <DefaultButton buttonText="Create Game" :class="tabName === 'create' ? 'bg-neutral-950' : ''" class="w-full! h-10! text-sm!" @click="tabName = 'create'" />
    </div>
    <HorizontalRule />
    <JoinGamePanel v-show="tabName === 'join'" />
    <CreateGamePanel v-show="tabName === 'create'" />
  </section>
</template>
