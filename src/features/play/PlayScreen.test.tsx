...
import { createFakeSceneApi, makeScene } from '../../test/scenes';
import { createDefaultServerSettings } from './serverSettings';
...
describe('PlayScreen', () => {
  ...
  it('keeps the Paint rail open and exposes compact Freeform and Polyline settings', async () => {
    ...
    expect(
      within(rail)
        .getAllByRole('button')
        .map((button) => button.getAttribute('aria-label')),
    ).toEqual(['Freeform paint', 'Polyline pen', 'Paint settings']);
    expect(
    ...
    expect(within(freeformSettings).getByLabelText('Edge')).toHaveValue(
      'hard',
    );
    expect(
      within(freeformSettings).queryByLabelText('Hardness'),
    ).not.toBeInTheDocument();
    await user.selectOptions(
      within(freeformSettings).getByLabelText('Edge'),
      'soft',
    );
  ...
  it('steps the active paint width with brackets without stealing form input', async () => {
    ...
    expect(within(freeform).getByLabelText('Width')).toHaveValue(20);
    const polylineWidth = within(polyline).getByLabelText('Width');
    expect(polylineWidth).toHaveValue(5);
