...
function stage(renderer: SceneRenderer) {
  return (renderer as unknown as { app: { stage: Container } }).app.stage
    .children[0];
}
...
describe('SceneRenderer', () => {
  it('attaches a transparent canvas so the play grid shows through', () => {
    expect(element.querySelector('canvas')).not.toBeNull();
    expect(stage(renderer)).toBeDefined();
  });
  ...
  it('shows a zoom-aware Freeform size ring and a Polyline nib cursor', () => {
    ...
    const brushCursor = (
      renderer as unknown as { brushCursor: SVGSVGElement }
    ).brushCursor;
    const camera = (
      renderer as unknown as { camera: { zoom: number } }
    ).camera;
    expect(element.style.cursor).toBe('none');
    element.dispatchEvent(
    ...
    expect(brushCursor.style.display).toBe('block');
    expect(brushCursor.style.left).toBe('420px');
    expect(brushCursor.style.top).toBe('310px');
    expect(brushCursor.style.width).toBe(
      `${Math.max(2, 24 * camera.zoom)}px`,
    );
    const diameterBeforeZoom = Number.parseFloat(brushCursor.style.width);
    element.dispatchEvent(
    ...
    expect(Number.parseFloat(brushCursor.style.width)).toBeGreaterThan(
      diameterBeforeZoom,
    );
    ...
    renderer.setInteraction({
      ...
      paintEnabled: true,
      paintKind: 'freeform',
      paintStyle: { ...paintStyle, strokeWidth: 48 },
    });
    const zoomAfterWheel = (
      renderer as unknown as { camera: { zoom: number } }
    ).camera.zoom;
    expect(brushCursor.style.width).toBe(
      `${Math.max(2, 48 * zoomAfterWheel)}px`,
    );

    element.dispatchEvent(
      pointerEvent('pointerleave', { button: -1 }),
    );
    expect(brushCursor.style.display).toBe('none');
    ...
    expect(element.style.cursor).toContain('crosshair');
    expect(brushCursor.style.display).toBe('none');
  });
